function [Top,getStats,meta] = make_A_eigs_top_preconditioned(sys,cfg,epsMean,budget)
...
% cycles reach a true-residual plateau below the configured plateau ceiling,
% the current shifted solve is explicitly marked PLATEAU ACCEPTED and returned.
% In addition, if a hard recovery/correction guard is reached while the true
% residual is already within the configured hard-guard plateau ceiling (1e-9 in
% Fig. 1(b,c)), the current solve is safely accepted instead of being turned
% into a run-stopping error.  The normal MINRES tolerance remains 1e-10.
%
...
% stagnation/maxit restarts and residual-correction solves.
    ...
    function Ycpu = applyT(Xcpu)
        budget.checkCanContinue('Method A Top entry');
    ...
    function [yg,info] = solve_shifted_system(rhs,outerIndex)
        ...
        guardNearTolCeiling = cfg.shift.guardNearToleranceCeiling;

        if ~(isscalar(guardNearTolCeiling) && isfinite(guardNearTolCeiling) && ...
                guardNearTolCeiling >= tol && guardNearTolCeiling <= plateauCeiling)
            error(['Invalid Method-A hard-guard acceptance ceiling. It must be finite ', ...
                   'and satisfy MINRES tolerance <= guard ceiling <= plateau ceiling.']);
        end

        % The requested MINRES tolerance remains tol=1e-10. Ordinary plateau
        % acceptance still requires repeated no-progress events and the wider
        % plateauCeiling. guardNearTolCeiling is consulted ONLY at a hard
        % recovery/correction guard. In the frozen Fig. 1(b,c) configuration
        % it equals 1e-9, so a harmless near-target plateau can never terminate
        % the whole eigensolve merely because the finite correction guard fires.

        while true
            if recoveryEvents > maxRecoveryEvents
                plateauDetected = plateauNoProgressEvents >= plateauMinEvents && ...
                    isfinite(finalExplicitRelres) && finalExplicitRelres <= plateauCeiling;
                nearToleranceAtGuard = isfinite(finalExplicitRelres) && ...
                    finalExplicitRelres <= guardNearTolCeiling;
                if plateauDetected || nearToleranceAtGuard
                    plateauAccepted = true;
                    plateauAcceptedResidual = finalExplicitRelres;
                    fprintf(['  [Outer Top %d] PLATEAU ACCEPTED at recovery guard: ', ...
                             'true relres=%.3e, guard ceiling=%.3e, ', ...
                             'plateau events=%d.\n'], ...
                            outerIndex,finalExplicitRelres,guardNearTolCeiling,plateauNoProgressEvents);
                    break;
                end
            ...
            segmentMaxit = budget.capMaxit(maxit);
            segmentTimer = tic;
            ...
            budget.addIterations(iterCPU,'Method A main MINRES');
            ...
            if flagCPU == 3
                ...
                if residualCorrections >= maxCorrectionCycles
                    plateauDetected = plateauNoProgressEvents >= plateauMinEvents && ...
                        isfinite(explicitRelres) && explicitRelres <= plateauCeiling;
                    nearToleranceAtGuard = isfinite(explicitRelres) && ...
                        explicitRelres <= guardNearTolCeiling;
                    if plateauDetected || nearToleranceAtGuard
                        plateauAccepted = true;
                        plateauAcceptedResidual = explicitRelres;
                        fprintf(['  [Outer Top %d] PLATEAU ACCEPTED at correction guard: ', ...
                                 'true relres=%.3e, guard ceiling=%.3e, ', ...
                                 'plateau events=%d, corrections=%d.\n'], ...
                                outerIndex,explicitRelres,guardNearTolCeiling, ...
                                plateauNoProgressEvents,residualCorrections);
                        break;
                    end
                ...
                budget.addIterations(corrInfo.iterations,'Method A residual-correction MINRES');
                ...
                if zeroProgressEvents >= maxZeroProgressEvents
                    plateauDetected = plateauNoProgressEvents >= plateauMinEvents && ...
                        isfinite(corrInfo.afterRelres) && corrInfo.afterRelres <= plateauCeiling;
                    nearToleranceAtGuard = isfinite(corrInfo.afterRelres) && ...
                        corrInfo.afterRelres <= guardNearTolCeiling;
                    if plateauDetected || nearToleranceAtGuard
                        plateauAccepted = true;
                        plateauAcceptedResidual = corrInfo.afterRelres;
                        fprintf(['  [Outer Top %d] PLATEAU ACCEPTED at zero-progress guard: ', ...
                                 'true relres=%.3e, guard ceiling=%.3e, ', ...
                                 'plateau events=%d.\n'], ...
                                outerIndex,corrInfo.afterRelres,guardNearTolCeiling,plateauNoProgressEvents);
                        break;
                    end
            ...
            if flagCPU == 1
                if iterCPU <= 0
                    if isfinite(explicitRelres) && explicitRelres <= guardNearTolCeiling
                        plateauAccepted = true;
                        plateauAcceptedResidual = explicitRelres;
                        fprintf(['  [Outer Top %d] PLATEAU ACCEPTED after zero-iteration MAXIT flag: ', ...
                                 'true relres=%.3e <= guard ceiling %.3e.\n'], ...
                                outerIndex,explicitRelres,guardNearTolCeiling);
                        yg = yNew;
                        break;
                    ...
                    error(['MINRES returned flag=1 with zero iterations in Outer Top %d. ', ...
                           'True relres=%.3e is above the %.3e fallback ceiling.'], ...
                          outerIndex,explicitRelres,guardNearTolCeiling);
                end
                if isfinite(previousExplicitRelres)
                    if explicitRelres >= previousExplicitRelres*(1-plateauProgressFraction)
            ...
            if isfinite(explicitRelres) && explicitRelres <= guardNearTolCeiling
                plateauAccepted = true;
                plateauAcceptedResidual = explicitRelres;
                fprintf(['  [Outer Top %d] NEAR-TARGET SOLVE ACCEPTED after MINRES flag=%d: ', ...
                         'true relres=%.3e <= guard ceiling %.3e.\n'], ...
                        outerIndex,flagCPU,explicitRelres,guardNearTolCeiling);
                yg = yNew;
                break;
            end
        ...
        function [yCorr,cinfo] = residual_correction(yBase,beforeRelres, ...
            ...
            corrMaxit = budget.capMaxit(maxit);
            corrTimer = tic;
