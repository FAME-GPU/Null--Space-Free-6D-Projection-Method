...
%BUILD_TAYLOR_GEOMETRY_EXP4 Device-resident local coordinates/powers.
    M=size(rPoints,1);
    nCenters=numel(blocks);
    centerId=zeros(M,1,'uint32');   % small indexing metadata on host
    ...
    geom=struct('centerIdHost',double(centerId),'centersDevice',centers, ...
