function [KrInvOp,getStats] = fig1a_make_nested_krinv_instrumented(sys,cfg,tolKr,tolM,maxitKr,maxitM,budget)
...
% The large-scale N=5 production setting uses tolKr=1e-10 and tolM=1e-12.
    ...
    function y = applyKr(z)
        ...
        [MyInv,flagM,relresM,iterM] = pcg( ...
            @(u)sys.mass.Mop(u),Vz,tolM,innerMaxit,[],[],y0);
        wait_for_gpu(MyInv);
        mElapsed = toc(mTimer);
